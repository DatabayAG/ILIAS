var ilPhantomJsWrapper =  (function () {
	...
	pub.addCookies = function(callback_function)
	{
		return phantom.callback(callback_function);
	};
	...
	pub.renderPageObject = function(src_file, out_file, delay, sess_obj, zoom)
	{
		pro.out_file	= out_file;
		pro.delay		= delay;
		pro.phantom_page.addCookie({
			'name'     : 'PHPSESSID',
			'value'    : sess_obj.session_id,
			'path'     : sess_obj.path,
...
var ilPhantomJsHelper =  (function () {
	...
	pro.configurePage = function() {
		...
		if (!pro.checkForViewportSetting()) {
			...
			pro.sess_obj = {
				'client_id'	: pro.json.client_id,
				'host'		: pro.json.host,
				'path'		: pro.json.path,
				'session_id': pro.json.session_id
			};
	...
	{
		ilPhantomJsWrapper.renderPageObject(pro.src_file, pro.out_file, pro.json.delay, pro.sess_obj, pro.json.zoom);
	};
